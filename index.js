...
let listenPhaseChanged = async () => {
	let minter = await paymentContract.bstMinter();
	let minterContract = new ethers.Contract(minter, BSTMinterABIJson.abi, provider);
	minterContract.on('PhaseChanged', (pool, phase, totalSupply) => {
		logger.info('=====================');
		logger.info('BSTMinter PhaseChanged:\t');
		logger.info('pool:\t' + pool);
		logger.info('phase:\t' + phase.toNumber());
		logger.info('totalSupply:\t' + ethers.utils.formatEther(totalSupply));
		logger.info('=====================');
	});
	liquidityContract.on('ZeroLPStaking', (pid, lastRewardBlock, blockNumber) => {
		logger.info('=====================');
		logger.info('LiqudityFarmingProxy ZeroLPStaking:\t');
		logger.info('pid:\t' + pid.toNumber());
		logger.info('lastRewardBlock:\t' + lastRewardBlock.toNumber());
		logger.info('blockNumber:\t' + blockNumber.toNumber());
		logger.info('=====================');
	});
};
let devBlockProducer = async () => {
	let wallet = wallets[1];
	while (true) {
		let bal = await wallet.getBalance();
		await wallet.sendTransaction({ to: wallet.address, value: bal.div(ethers.BigNumber.from('10000')).toHexString() });
		delay(500);
	}
};

let farmingPool2 = async (ms) => {
	let task = async () => {
		let wallet = wallets[0];
		let pool = await paymentContract.pool();
		let pool2Contract = new ethers.Contract(pool, BStablePoolABIJson.abi, provider);
		let lpBal = await pool2Contract.balanceOf(wallet.address);
		if (lpBal.gt(0)) {
			logger.info('Deposit LP.')
			await pool2Contract.connect(wallet).approve(liquidityContract.address, lpBal);
			await liquidityContract.connect(wallet).deposit(1, lpBal);
		} else {
			let userInfo = await liquidityContract.userInfo(1, wallet.address);
			if (userInfo.amount.gt(0)) {
				logger.info('Withdraw LP.')
				await liquidityContract.connect(wallet).withdraw(1, 0);
			} else {
				logger.info('Add LP.')
				let amts = [ethers.utils.parseEther('10000'), ethers.utils.parseEther('10000'), ethers.utils.parseEther('10000')];
				await usdcContract.connect(wallet).approve(pool, ethers.utils.parseEther('10000'));
				await busdContract.connect(wallet).approve(pool, ethers.utils.parseEther('10000'));
				await usdtContract.connect(wallet).approve(pool, ethers.utils.parseEther('10000'));
				await pool2Contract.connect(wallet).add_liquidity(amts, 0);
			}
		}
	};
...
switch (funName) {
	...
	case 'devBlockProducer':
		logger.info('BST Farmer - devBlockProducer');
		devBlockProducer();
		break;
	case 'farmingPool2':
		logger.info('BST Farmer - farmingPool2');
		farmingPool2(process.argv[4] * 1000);
		break;
